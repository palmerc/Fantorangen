//
//  WBSFantorangenEpisodesSectionTableViewCell.m
//  Fantorangen
//
//  Created by Cameron Palmer on 06.01.14.
//  Copyright (c) 2014 Wolf and Bear Studios. All rights reserved.
//

#import "WBSFantorangenEpisodesSectionTableViewCell.h"

NSString *const kWBSFantorangenEpisodesSectionTableViewCellReuseIdentifier = @"WBSFantorangenEpisodesSectionTableViewCellReuseIdentifier";



@implementation WBSFantorangenEpisodesSectionTableViewCell

- (void)awakeFromNib
{
    [super awakeFromNib];
    
    self.backgroundColor = [UIColor lightGrayColor];
}

@end
